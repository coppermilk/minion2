...
_MENU = (
    ...
    '  config              - list every setting and its value\n'
...
def _unknown(key: str) -> str:
    ...
    return f'unknown setting {key!r}; send "config" for the list'
...
class _Moderator:
    ...
    def _config(self) -> str:
        """List every setting with its current value and one-line help."""
        admin = admin_config(self.cfg.state)
        lines = ['config (set <key> <value>, reset <key>):']
        lines += [
            f'  {s.key} = {admin.get(s.key)}   -- {s.help}' for s in SETTINGS
        ]
        return '\n'.join(lines)
    ...
    def _whois(self, chat: str) -> str:
        ...
        return f'{chat} -> {name}' if name else f'{chat}: unknown / no access'
